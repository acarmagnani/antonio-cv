...
- **content_base.yaml**:         Source of truth. Full history with tagged bullets. Never edit during tailoring. Contains `profiles` (array of tagged variants), `experience` (bullets as `{text, tags}`), and `projects` (same).
- **select_cv.py**:              Selection script. Reads `selected_tags.yaml` + `content_base.yaml`, picks the best-matching profile and all bullets with matching tags, outputs `content_tailored.yaml` with plain strings.
...
- **templates/cv.html**:         Renders a CV from content_base.yaml + the tailored YAML resolved via active_application.txt.
- **assets/**:                   CV and cover letter CSS.
...
- **selected_tags.yaml**:        Tags chosen by Claude for this application. Reviewed by Antonio before the script runs.
- **content_tailored.yaml**:     Generated by `select_cv.py`. Contains only profile, experience, and projects as plain strings. Stable sections (meta, education, skills) live in content_base.yaml and are merged at render time.
...
- `templates/cv.html` reads `active_application.txt` from the repo root to find the active application folder name.
- It then loads `applications/[pointer]/content_tailored.yaml` from that folder.
- If the pointer file is missing or empty, it falls back to rendering from `content_base.yaml` directly (handles both plain-string and object-format bullets).
...
- `.claude/skills/cv-tailoring/SKILL.md`: the CV tailoring workflow (tags → script → editorial pass).
- `.claude/skills/cover-letter/SKILL.md`: cover letter writing, builds on an existing application's tailored CV.
