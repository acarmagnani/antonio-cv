...
- **system/working-preferences.md**: How Antonio wants Claude to work here (reply language, CV length bias, frozen submitted applications, job-discovery approach, multi-machine git caveats). Lives in the repo because Claude Code's own memory is per-machine and does not survive a new laptop. Read it at the start of a session alongside `context.md`.
