...
- **system/show_blocks.py**:     Writes every preset block out as resolved text to `preset_previews/blocks.md`, each bullet tagged with its id. The fast, always-fresh way to read a whole block and judge whether it points one direction: edit `content_base.yaml`, re-run, the file updates in place. Generated, never edited by hand.
